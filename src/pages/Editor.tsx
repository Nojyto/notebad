...
const EditorPage = () => {
  ...
  const highlightMatch = (index: number, length: number) => {
    ...
    if (textArea && index >= 0) {
      ...
      const startLine =
        textArea.value.substring(0, index).split("\n").length - 1;
      const lineHeight = parseFloat(
        getComputedStyle(textArea).lineHeight || "20px"
      );
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      ...
      >
        <div className="flex items-center border-border px-1">
          <div
            ref={tabListRef}
            className="flex-1 overflow-x-auto scrollbar-small"
                ...
                >
                  {({ selected }) => (
                    ...
                    >
                      ...
                      <div
                        className={`mx-1 pb-0.5  ${
                          selected ? "text-red-200" : "text-red-500"
                        }`}
                      >
                        {!tab.isSaved && "●"}
                      </div>
          ...
          <button
            onClick={() => addTab()}
            className="w-8 h-8 ml-2 bg-accent text-accent-foreground rounded-xl flex items-center justify-center text-justify hover:bg-destructive"
          >
            +
          </button>
        </div>
        <TabPanels className="flex-1 flex flex-col h-full">
          {tabs.map((tab: TabData, index: number) => (
            <TabPanel
              key={tab.id}
              className="relative flex-1 flex flex-col bg-popover text-popover-foreground"
            >
              <SearchBar
                ...
                onContentReplace={(updatedContent) => {
                  updateTabContent(activeIndex, updatedContent);
                }}
